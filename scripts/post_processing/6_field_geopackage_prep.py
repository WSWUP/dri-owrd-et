#--------------------------------
# Name:    6_field_geopackage_prep.py
...
"""
Convert individual field summary CSVs into a geopackage
...
def main(ini_path=None):
    ...
    logging.info('\nField-Level Geopackage preparation (Step 6)')
    ...
    try:
        static_df = pd.read_csv(os.path.join(in_path, f'or_openet_etdemands_monthly_water_year_shift_1mo_{start_yr}_final.csv.gz'), index_col=unique_id, 
    ...
    for year in year_list:
        ...
        logging.info(f'\nProcessing {year} field level data')
    
        try:
            # timeseries dataframe for the current year in the loop
            select_year_df = pd.read_csv(os.path.join(in_path, f'or_openet_etdemands_monthly_water_year_shift_1mo_{year}_final.csv.gz'), dtype={unique_id: str}, index_col=unique_id, 
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Field-Level Geopackage Preparation (Step 6)',
