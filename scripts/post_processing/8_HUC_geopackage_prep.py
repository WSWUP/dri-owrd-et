...
def main(ini_path=None):
    ...
    for huc_level in huc_level_list:
        ...
        for src_type in src_type_list:
            ...
            try:
                # read each HUC stats dataframe table
                gdf_irr = gpd.read_file(os.path.join(in_path, f'or_openet_{huc_level.lower()}_irrigated_{src_type}.shp')).set_index(f'{huc_level}_code')
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='HUC Watershed Geopackage Preparatation of HUC Level Volumes (Step 8)',
