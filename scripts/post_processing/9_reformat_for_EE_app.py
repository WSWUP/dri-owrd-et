...
def main(ini_path=None):
    ...
    logging.info('\nField-Level Geopackage reformatting for Earth Engine (Step 9)')
    ...
    for k, v in year_blocks.items():
        ...
        # fill nan with 0's
        df_ts_mo = df_ts_mo.fill_nan(0)
        df_ts_an = df_ts_an.fill_nan(0)
        
        # export monthly and annual timeseries
        df_ts_mo.write_csv(os.path.join(out_path, f'oregon_field_monthly_summary_{k}_{v}.csv'), separator=',')
        df_ts_an.write_csv(os.path.join(out_path, f'oregon_field_annual_summary_{k}_{v}.csv'), separator=',')
        
        print(f'finished exporting reformatted data for {k}-{v}')
    ...
    if end_yr - start_yr <= 9:
        ...
        # list of filenames for process
        filenames = [file for file in os.listdir(out_path) if 'annual_summary_' in file]

        # build the merged annual table from the subsets
        df_ts_an_mpre = pl.DataFrame([])
        for filename in filenames:
        
            try:
                # read the dataframe
                df_a = (
                    pl.scan_csv(os.path.join(out_path, filename))
                        .collect(engine='auto')
                        .select(pl.all())
                )       
        
                # concatenate dataframes
                data_ts_an = [df_ts_an_mpre, df_a]
                df_ts_an_mpre = pl.concat(data_ts_an, how='vertical')
        
        
            except Exception as e:
                print(e)
        
    df_ts_an_m = df_ts_an_mpre.select(pl.all().sort_by([unique_id, 'Date']))
    
    # calculate a mean annual dataframe to export grouped by DRI_ID, but first replace 0's with nans before calculating means
    df_mean_an_pre = (
        df_ts_an_m
            .with_columns(
                # pl.col(pl.Float64).replace(0, np.nan) # nan resulted in incorrect mean annual calculation
                pl.col(pl.Float64).replace(0, None)
            )
    )
    
    # prep stat settings for all columns, CDL needs to use a mode statistic
    mode_cols = ['CDL', 'IRR STATUS FLAG', 'ITYPE', 'srctype']
    ...
    df_mean_an = (
        df_mean_an_pre
            ...
            .agg([pl.col('CDL').mode().first().alias('CDL')]
                +
                 [pl.col('IRR STATUS FLAG').mode().first().cast(pl.Int32).alias('IRR STATUS FLAG')]
                +
                 [pl.col('ITYPE').mode().first().alias('ITYPE')]
                +
                 [pl.col('srctype').mode().first().alias('srctype')]
                +
            ...
            # .fill_nan(0) # a full replacment of 0's with nans resulted in incorrect mean annual calculation
            .fill_null(0)
    )
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Field-Level Geopackage Reformatting for Earth Engine (Step 9)',
