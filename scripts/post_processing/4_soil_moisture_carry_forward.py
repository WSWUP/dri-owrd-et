...
in the previous step (i.e., 1985-2024).
...
def main(ini_path=None):
    ...
    if not year_list[0] == 1985:
        logging.warning(
            '\nWARNING: Soil Moisture Carry Forward should begin at the study period start (i.e., 1985)')
        sys.exit()
    ...
    def compute_month(df, mo, cal_year, ws_prev):
        ...
        # Cumulative deficit carry-forward
        df[ws_c_col] = (ws_prev + df[irr_col]).clip(upper=0)
    
        # Adjusted irrigation cannot be negative
        df[irr_adj_col] = (df[irr_col] + ws_prev).clip(lower=0)
    ...
    def soil_moisture_carry_forward(
        ...
        for i, wy in enumerate(year_list):
            ...
            # 1️Drop WS_ columns but keep WS_C_
            ws_only_cols = [
