#--------------------------------
# Name:    ee_monthly_huc_evap_climo_zonal_stats.py
...
It exports monthly small pond evaporation climatology summaries as CSV tables
for the historical study period 1985-2024 (Nov-Oct months)
...
def main(ini_path=None):
    ...
    def climoColl(month):
        """calculates the climatology for a month
    
        Args:
            month: earth engine month integer
        
        Returns:
            earth engine climatology image with updated properties
        
        """
    
        # calculate the average of all monthly values across the project period
        img = (
            joined
        ...
        return img.set({
            'system:index': ee.Number(month).format('%02d'),
            'month': month,
        })
    
    climo_coll = ee.ImageCollection(months.map(climoColl))
    
    # convert the image collection to an image with numerous bands
    climo_coll_bnds = climo_coll.toBands()
    
    # getInfo call to get the list of band names as a python list
    climo_coll_bnds_list = climo_coll_bnds.bandNames().getInfo()
    ...
    if out_location == 'google_drive':
        ...
        out_table_huc_climo = ee.batch.Export.table.toDrive(**{
            'collection': ee.FeatureCollection(huc_stats_out),
            'description':f'OR_{huc_code}_Summary_Export_monthly_climo_small_pond_evap',
            'fileNamePrefix': f'or_gridmet_huc{huc_c}_summaries_monthly_climo_eto_small_pond_evap_inches',
            'folder': gdrive_folder_name,
            'fileFormat': 'CSV',
            'selectors': [f'{huc_code}_code', f'{huc_code}_name'] + climo_coll_bnds_list,
        })
        ...
        out_table_huc_climo = ee.batch.Export.table.toCloudStorage(**{
            'collection': ee.FeatureCollection(huc_stats_out),
            'description':f'OR_{huc_code}_Summary_Export_monthly_climo_small_pond_evap',
            'bucket': bucket,
            'fileNamePrefix': f'{bucket_path}/or_gridmet_huc{huc_c}_summaries_monthly_climo_eto_small_pond_evap_inches',
            'fileFormat': 'CSV',
            'selectors': [f'{huc_code}_code', f'{huc_code}_name'] + climo_coll_bnds_list,
        })
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Earth Engine HUC-Level Monthly Evaporation Climatology Zonal Stats Table Export',
