...
def main(tgt_path, field_name='HUC12'):
    ...
    huc12_geojson = os.path.join(os.getcwd(), 'WBDHU12_CONUS.shp')
    huc12_field_name = 'huc12'
    ...
    logging.info('\nReading HUC12 data')
    logging.info('  {}'.format(huc12_geojson))
    huc12_ds = ogr.Open(huc12_geojson, 0)
    huc12_lyr = huc12_ds.GetLayer()
    huc12_osr = huc12_lyr.GetSpatialRef()

    # Project the input extent to the MGRS projection
    huc12_tx = osr.CoordinateTransformation(tgt_osr, huc12_osr)
    tgt_extent.Transform(huc12_tx)
    logging.debug('  Extent: {}'.format(tgt_extent))

    # Filter the MGRS to the input extent
    huc12_lyr.SetSpatialFilter(tgt_extent)

    # Save the HUC12 geometry to a dictionary by HUC12 ID
    huc12_dict = {}
    for huc12_ftr in huc12_lyr:
        huc12_id = huc12_ftr.GetField(huc12_field_name)
        huc12_dict[huc12_id] = huc12_ftr.GetGeometryRef().Clone()
    huc12_ds = None
    ...
    # Add the HUC12 tile field if it doesn't exist
    ...
    for tgt_ftr in tgt_lyr:
        ...
        try:
            ...
            tgt_pnt.Transform(huc12_tx)
            # logging.debug('    {}'.format(tgt_pnt))
            for huc12_id, huc12_geom in huc12_dict.items():
                if tgt_pnt.Intersects(huc12_geom):
                    # logging.debug('    {}'.format(huc12_id))
                    tgt_ftr.SetField(field_name, huc12_id)
...
def arg_parse():
    ...
    parser = argparse.ArgumentParser(
        description='Add HUC12 tile field to an existing shapefile',
    ...
    parser.add_argument(
        '--field', default='HUC12', type=str, help='HUC12 tile field name')
    parser.add_argument(
